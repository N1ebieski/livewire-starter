...
use App\Livewire\Components\Admin\User\DeleteComponent;
...
final class DeleteComponentTest extends TestCase
{
    ...
    public function test_authorize_in_render_method(?Closure $authUser, int $responseCode): void
    {
        if ($authUser) {
            ...
            Livewire::test(DeleteComponent::class, ['user' => $authUser])->assertStatus(Response::HTTP_FORBIDDEN);
        }

        $user = User::factory()->user()->create();

        Livewire::test(DeleteComponent::class, ['user' => $user])->assertStatus($responseCode);
    }
}
